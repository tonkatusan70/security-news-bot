...
# ✅ 設定された投稿時間
ALLOWED_HOURS = [7, 12, 20]  # 07:00, 12:00, 20:00（JST）

# ✅ コマンドライン引数の設定
parser = argparse.ArgumentParser()
parser.add_argument("--test", action="store_true", help="テストモードで投稿する")
...
def post_to_x():
    ...
    # ✅ テストモードでない場合、指定時間以外は投稿しない
    if not args.test and now_jst.hour not in ALLOWED_HOURS:
        print(f"⏳ 現在は投稿時間ではありません。（現在: {now_jst.strftime('%H:%M JST')}）")
        return
